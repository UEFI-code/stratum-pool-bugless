...
var BlockTemplate = module.exports = function BlockTemplate(maxDifficulty, jobId, rpcData, publicKey, extraNoncePlaceholder, reward, txMessages){
    ...
    this.prevHashReversed = util.reverseByteOrder(new Buffer.from(String(rpcData.previousblockhash, 'hex'))).toString('hex');
    this.transactionData = Buffer.concat(rpcData.transactions.map(function(tx){
        return new Buffer.from(String(tx.data, 'hex'));
    }));
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce){
        ...
        var header =  new Buffer.alloc(80);
        var position = 0;
