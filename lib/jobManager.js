...
var ExtraNonceCounter = function(poolHoldLength){
    //console.log(poolHoldLength)
    this.next = function(){
        return crypto.randomBytes(poolHoldLength).toString('hex');
    };
};
...
var JobManager = module.exports = function JobManager(maxDifficulty, options){
    ...
    this.extraNonceCounter = ExtraNonceCounter(options.poolHoldLength);
    this.extraNonce2Size = this.extraNoncePlaceholder.length - options.poolHoldLength;
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, workerName){
        ...
        var shareDiff = maxDifficulty.toNumber() / headerBigNum.toNumber()

        var blockDiffAdjusted = maxDifficulty.toNumber() / job.target.toNumber();

        //Check if share is a block candidate (matched network difficulty)
        if (/*job.target.ge(headerBigNum)*/true){
